    ...
    setTimeout(() => {
      canvasConfigs.forEach((config, index) => {
        ...
        const canvasContainerEl = document.getElementById(
          `canvas-${index}-container`,
        );
        const linkEl = canvasContainerEl.getElementsByTagName("a")[0];
        const href = `/artomata/pentaflower/create?rings=${config.rings}&generation=${config.startGeneration}&color1=${encodeURIComponent(config.colors[0])}&color2=${encodeURIComponent(config.colors[1])}&color3=${encodeURIComponent(config.colors[2])}`;
        imgEl.src = `/artomata/assets/img/${name.replace(/\s/g, "_")}.png`;
